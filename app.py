...
app = Flask(__name__)
CORS(app)
app.config['SECRET_KEY'] = 'ducnx299'
...
def get_description(product_name):
    ...
    print(f"Current session product name: {session.get('last_product_name', 'None')}")
    # Lưu tên sản phẩm trong session (nếu chưa có)
    if 'last_product_name' not in session or session['last_product_name'] is None:
        session['last_product_name'] = product_name.strip()  # Lưu tên sản phẩm nguyên bản vào session
        print(f"Saving full product name in session: {product_name}")  # Debugging để kiểm tra
    ...
    # Kiểm tra từ khóa "số lượng" hoặc "còn"
    ...
    # Kiểm tra từ khóa "chất liệu giày"
    if any(keyword in product_name.lower() for keyword in ["chất liệu giày"]):
        ...
        if chat_lieu:
            ...
            return {"response": "Sản phẩm chưa được tìm thấy chất liệu."}
    
    # Kiểm tra từ khóa "công nghệ giày"
    if any(keyword in product_name.lower() for keyword in ["công nghệ giày"]):
        ...
        if cong_nghe:
            ...
            return {"response": "Sản phẩm chưa được tìm thấy công nghệ."}
    ...
    if "mô tả" in product_name.lower():
        ...
        for index, row in df.iterrows():
            if fuzz.partial_ratio(product_name_cleaned.lower(), row['product_name'].lower()) > 70:
                session['last_product_name'] = row['product_name']  # Lưu lại tên sản phẩm
                print(f"Before setting session, session content6: {session['last_product_name'] }")
                return {"response": f"Mô tả của {row['product_name']} là: {row['description']}"}
        return {"response": "Sản phẩm không được tìm thấy."}
    
    # Kiểm tra các từ khóa liên quan đến "công nghệ", "giá", "chất liệu" (cho các câu hỏi lặp lại)
    keywords_for_info = ["giá", "chất liệu", "công nghệ"]
    for keyword in keywords_for_info:
        if keyword in product_name.lower():
            if f"{keyword} của nó" in product_name.lower() and session['last_product_name']:
                # Tìm kiếm thông tin theo sản phẩm đã hỏi trước đó
                product_name_cleaned = session['last_product_name'].lower()
                print(f"product_name_cleaned session: {product_name_cleaned}")
                if "giá" in keyword:
                    sale_price = get_sale_price(product_name_cleaned)
                    if sale_price:
                        print(f"Before setting session, session content5: {session['last_product_name'] }")
                        return {"response": f"Giá của {session['last_product_name']} là {sale_price} VND."}
                    else:
                        return {"response": "Sản phẩm chưa được tìm thấy giá."}
                
                if "chất liệu" in keyword:
                    chat_lieu = get_chat_lieu(product_name_cleaned)
                    if chat_lieu:
                        print(f"Before setting session, session content4: {session['last_product_name'] }")
                        return {"response": f"Chất liệu của {session['last_product_name']} là {chat_lieu}."}
                    else:
                        return {"response": "Sản phẩm chưa được tìm thấy chất liệu."}
                
                if "công nghệ" in keyword:
                    cong_nghe = get_cong_nghe(product_name_cleaned)
                    if cong_nghe:
                        print(f"Before setting session, session content3: {session['last_product_name'] }")
                        return {"response": f"Công nghệ của {session['last_product_name']} là {cong_nghe}."}
                    else:
                        return {"response": "Sản phẩm chưa được tìm thấy công nghệ."}
            else:
                # Xử lý cho câu hỏi mới về giá, chất liệu, công nghệ
                product_name_cleaned = product_name.lower()
    ...
    # Nếu không có từ khóa "giá", "mô tả", "chất liệu", "công nghệ", kiểm tra tên sản phẩm
    if any(keyword in product_name.lower() for keyword in ["mẫu", "sản phẩm"]):
        product_info = get_product_info_by_name(product_name)
        
        if product_info:
            session['last_product_name'] = product_name  # Lưu lại tên sản phẩm
            print(f"Before setting session, session content2: {session['last_product_name'] }")
    ...
    for index, row in df.iterrows():
        ...
        if ratio > highest_ratio:
            ...
            session['last_product_name'] = row['product_name']  
    
    # Nếu fuzzy search tìm thấy tỷ lệ phù hợp cao (trên 70%)
    if highest_ratio > 70:
        print(f"Before setting session, session content: {session}")
        # session['last_product_name'] = row['product_name']  
        print(f"After setting session, session content: {session['last_product_name']}")
